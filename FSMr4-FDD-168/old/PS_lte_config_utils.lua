	...
	for enbCounter = 1, numEnb do
		...
		for i = 1, table.getn(ordered_enbs_keys) do
            ...
            if enbTable["type"]=="simulated" then
                DAEMON_UEC_IP=enbTable["daemonIp"]
                DAEMON_UEC_PORT=enbTable["daemonPort"]
            end
                exec("set_enb"..
                    " type="..enbTable["type"]..
                    " id="..enbId..
				...
				for i = 1, table.getn(ordered_uec_keys) do
					...
					if paramKey == "cells" then
						...
						for i = 1, table.getn(ordered_cells_keys) do
							...
							UEC1_PROXY_ADDR = iphy_ip_table[i]..":"..UEC_TABLE_value[enbTable["uecName"]]["uecProxyPort"]
                            if enbTable["type"]=="simulated" then
...
function set_uec_parameters( UEC_TABLE, PGW_TABLE )
	...
	for i = 1, table.getn(ordered_keys) do
		...
		exec("set_uec"..
			...
			" peer="..uecTable["uecServerIp"]..":"..uecTable["uecProxyPort"]..
			" peer_uplane="..uecTable["UEC_PIP_UPLANE"]..
			" retries=unlimited"
		)
		uecId = uecId + 1
	end
