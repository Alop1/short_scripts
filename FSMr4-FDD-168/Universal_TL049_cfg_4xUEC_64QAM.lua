...
local ENB_TABLE = {
    ...
    ENB_2 = {
        ...
        enbIdMacro = '0x00032',
        s1 = eNB2_IP,
        -- daemonIp = server_deamonip,  -- only if different than in EPC_TABLE
        -- daemonPort = server_deamonport,-- only if different than in EPC_TABLE
        proxy_IP = '127.0.0.1',
        proxy_port = 2154,
        tac = "123",
        cells = {{1}},
...
local UE_CONFIG_PARAMS = {
{
    ...
    ue_group_id = {1, 2, 3}, -- for compability with old tests, if not set then 100*n+cellId
    ueCount = 1000,
...
{
    cell_id = {1, 2, 3}, -- if not set use all
   ue_group_id = {4, 5, 6}, -- for compability with old tests, if not set then 100*n+cellId
    ueCount = 1000,
    encryption = 'snow3g+aes',
    ue_group_parametars = "release=13 category-dl=12 category-ul=13 ul_64QAM_supported=true",
