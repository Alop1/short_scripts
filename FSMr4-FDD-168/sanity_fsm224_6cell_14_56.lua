--************************************************************************************
require "configs/hw_vars_224_6cell"


exec('debug_log level=3')
exec('debug_log l3sim=1 level=3')
		...
		for cell_i = 1, CELL_COUNT[proxy_i] do
			...
			if is_hw then
				if PCI_LIST and PCI_LIST[cell_i + id_offset] then
					...
					exec('set_cell id='..CELL_ID_ARRAY[proxy_i][cell_i]..' enb=1 pci='..PCI_LIST[cell_i + id_offset]..' tac='..ENB_UT_HW_TAC..' proxy='..PROXY_ADDR[proxy_i]..' eci='..eci)

				else
					exec('set_cell id='..CELL_ID_ARRAY[proxy_i][cell_i]..' enb=1 tac='..ENB_UT_HW_TAC..' proxy='..PROXY_ADDR[proxy_i]..' eci='..eci)
				...
				if PCI_LIST and PCI_LIST[cell_i + id_offset] then
					exec('set_cell id='..CELL_ID_ARRAY[proxy_i][cell_i]..' enb=1 pci='..PCI_LIST[cell_i + id_offset]..' tac='..ENB_UT_SIM_TAC..' proxy=$EPC_SERVER_IP:'..tostring(random_port - cell_i - id_offset)..' eci='..eci)
...
--------------------------------------------
    ...
    for uec_id = 1, #PIP_ADDR do
    	for cell_id = 1, CELL_COUNT[uec_id] do
    		cell_id_from_array = CELL_ID_ARRAY_LINED[cell_id]

            exec('set_ue_group simulated id='..cell_id .. uec_id ..' uec='.. uec_id.. ' category=4  eia=null eea=null cell='..CELL_ID_ARRAY[uec_id][cell_id]..' pdn='..uec_id)
...
--              exec('ue_param  id='.. start_id + temp_ue_id ..' supported_bands=1,3,4,6,19,21,28')
		...
		--exec('ue_cqi_param id='.. start_id + temp_ue_id ..' wb=15:15:15 rank=4 ignore_codebook_subset_restriction=on')
            end
 ...
 seq_nr=0
    for uec__id=1,#PIP_ADDR do
        for cell_id=1,CELL_COUNT[uec__id] do
